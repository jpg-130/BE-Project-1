...
def main():
    ...
    camera = cv2.VideoCapture("D:/sem7/Project/Implementation/Write_jigyasa.mp4")

    width  = camera.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
    height = camera.get(cv2.CAP_PROP_FRAME_HEIGHT) # float
    # region of interest (ROI) coordinates
    top, right, bottom, left = 10, 10, int(width-10), int(height-10)
            ...
            if num_frames < 30:
                ...
                if hand is not None:
                    ...
                    if start_recording:
                        ...
                        cv2.imwrite("D:/sem7/Project/Implementation/VideoFrame/ok/ok" + str(image_num) + '.png', thresholded)
                        image_num += 1
                        print(image_num)
